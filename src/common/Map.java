...
public class Map {
     ...
     */
    public Map(ArrayList<IObstacle> obstacles) {
     ...
     */
    private IObstacle getObstacleAtLocation(int x, int y) {
    ...
    public String getSolvedMap(Location start, Location target) {
        ...
        // Find the path
        IGridPathFinder pathFinder = new BFSPathFinder(this);
        ...
        for (int y = topLeft.getY(); y <= bottomRight.getY(); y++) {
            for (int x = topLeft.getX(); x <= bottomRight.getX(); x++) {
                ...
                // 2. Check obstruction
                IObstacle obstructingObstacle = getObstacleAtLocation(x, y);
